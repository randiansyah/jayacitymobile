...
class Pembayaran_brand extends Admin_Controller {
  public function __construct()
  {
    parent::__construct();
    $this->load->model('Laporan_pembayaran_brand_model','brand'); 
  ...
  public function index()
  {
    ...
    if($this->data['is_can_read']){
      $this->data['content'] = 'admin/laporan_pembayaran1/list_brand_v';   
    }else{
        ...
        if(!empty($datas))
        {
            foreach ($datas as $key=>$data)
                ...
                if ($this->data['is_can_edit']) {
                  $edit_url = "<a href='" . base_url() . "Pembayaran_brand/view/" . $data->id . "' class='btn btn-primary btn-sm white'><i class='fa fa-eye'></i> Lihat</a>";
                } else {
                  $edit_url = "";
                }
  ...
  public function view($id)
  {
    $this->load->helper('url');
    if ($this->data['is_can_read']) {
      $this->data['content'] = 'admin/laporan_pembayaran1/list_detail_v';
  ...
  public function dataListBrand($id)
  {
    $columns = array(
      0 => '',
    ...
    $limit = 0;
    $start = 0;
    $jatuh_tempo = $this->brand->getOneJatuh_tempo();
    $set = (!empty($jatuh_tempo)) ? $jatuh_tempo->set_hari : "";
    $setting_hari = "+ interval " . $set . " day";

    $where['s.merek'] = $id; 
    $totalData = $this->brand->getCountAllByBrand($limit, $start, $search, $order, $dir, $where, $setting_hari);
    ...
    if ($isSearchColumn) {
      $totalFiltered = $this->brand->getCountAllByBrand($limit, $start, $search, $order, $dir, $where, $setting_hari);
    } else {
      $totalFiltered = $totalData;
    }
    ...
    $limit = $this->input->post('length');
    $start = $this->input->post('start');
    $datas = $this->brand->getAllByBrand($limit, $start, $search, $order, $dir, $where, $setting_hari);
