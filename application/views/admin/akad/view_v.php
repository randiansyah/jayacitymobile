...
<section class="content">
  <div class="box box-default color-palette-box">
    <div class="box-header with-border">
      <h3 class="box-title"><i class="fa fa-tag"></i>Rincian Transaksi</h3>
    </div>
        ...
        <div class="row">
            ...
            <div class="form-group">
              <div class="form-group row">
                <div class="col-md-8"> <label for="">INV PELANGGAN</label>
                  <select id="id_invoice" name="id_invoice" class="form-control select2" value="<?php echo $data->id_invoice ?>">
                    <option value="">Pilih salah satu</option>
                    <?php
                    foreach ($pelanggan as $key => $val) {
                      if ($data->id_invoice == $val->id_invoice) {
                    ?>
                        <option value="<?php echo $val->id_invoice; ?>" selected>INV : <?php echo $val->id_invoice . ' - ' . $val->nama ?></option>
                      <?php } else {
                      ?>
                        <option value="<?php echo $val->id_invoice; ?> ">INV : <?php echo $val->id_invoice . ' - ' . $val->nama ?></option>
                    <?php }
                    }
                    ?>
                  </select>
              ...
              <div class="form-group">
                ...
                <input class="form-control" name="nomor_akad" autocomplete="off" value="<?php echo $data->nomor_akad ?>">
              </div>
              <div class="form-group">
                <label for="">TANGGAL Transkasi</label>
                <input class="form-control datepicker" id="tgl_akad" name="tgl_akad" value="<?php echo date("d-m-Y", strtotime($data->tgl_akad)) ?>">
              </div>
              <div class="form-group">
                <label for="">TGL JATUH TEMPO</label>
                <input class="form-control datepicker" autocomplete="off" name="tgl_jatuh_tempo" autocomplete="off" value="<?php echo date("d-m-Y", strtotime($data->tgl_jatuh_tempo))  ?>">
              </div>
              <div class="form-group">
          ...
          <div class="col-md-4">
            ...
            <div class="form-group">
              ...
              <input class="form-control harga" autocomplete="off" id="harga_jual" name="harga_jual" autocomplete="off" value="<?php echo "Rp. " . number_format($data->harga_jual, 0, ",", ".") ?>">
            </div>
            <div class="form-group">
              <label for="">UANG MUKA</label>
              <input class="form-control harga" autocomplete="off" id="uang_muka" name="uang_muka" autocomplete="off" value="<?php echo "Rp. " . number_format($data->uang_muka, 0, ",", ".") ?>">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <input class="form-control" type="number" id="bunga" name="bunga" autocomplete="off" value="<?php echo $data->bunga ?>">
            </div>
            </div>
            <div id="bungaOption">
            <div class="form-group">
              <label for="">BUNGA MANUAL/BULAN</label>
              <input class="form-control harga" id="bungaManual" name="bungaManual" autocomplete="off" value="<?php echo $data->bunga ?>">
            </div>
            </div>
            <div class="form-group">
              <label for="">LAMA CICILAN</label>
              <select name="lama_cicilan" class="form-control select2" id="lama_cicilan" value="<?php echo $data->lama_cicilan ?>">
                <option value="">Pilih</option>
                <?php
                foreach ($lama_cicilan as $key => $val) {
                  if ($data->lama_cicilan == $val->nama) {
                ?>
                    <option value="<?php echo $val->nama; ?>" selected><?php echo $val->nama ?> / Bulan</option>
                  <?php } else {
                  ?>
